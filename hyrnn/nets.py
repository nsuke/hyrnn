...
def one_rnn_transform(W, h, U, x, b, c):
    ...
    Wh_plus_Ux = pmath.mobius_add(W_otimes_h, U_otimes_x, k=to_k(c, W_otimes_h))
...
def mobius_gru_cell(
    ...
    z_t = pmath.logmap0(one_rnn_transform(W_hz, hx, W_iz, input, b_z, c), k=to_k(c, hx)).sigmoid()
    r_t = pmath.logmap0(one_rnn_transform(W_hr, hx, W_ir, input, b_r, c), k=to_k(c, hx)).sigmoid()

    rh_t = pmath.mobius_pointwise_mul(r_t, hx, k=to_k(c, hx))
    h_tilde = one_rnn_transform(W_hh, rh_t, W_ih, input, b_h, c)

    if nonlin is not None:
        h_tilde = pmath.mobius_fn_apply(nonlin, h_tilde, k=to_k(c, hx))
    delta_h = pmath.mobius_add(-hx, h_tilde, k=to_k(c, hx))
    h_out = pmath.mobius_add(hx, pmath.mobius_pointwise_mul(z_t, delta_h, k=to_k(c, hx)), k=to_k(c, hx))
    return h_out
...
def mobius_gru_loop(
    ...
    if not hyperbolic_hidden_state0:
        hx = pmath.expmap0(h0, k=to_k(c, h0))
    else:
        hx = h0
    if not hyperbolic_input:
        input = pmath.expmap0(input, k=to_k(c, h0))
    outs = []
        ...
        **kwargs
        ...
        if self.bias is not None:
            if hyperbolic_bias:
                ...
                with torch.no_grad():
                    self.bias.set_(pmath.expmap0(self.bias.normal_() / 4, k=to_k(c, self.bias)))
        with torch.no_grad():
